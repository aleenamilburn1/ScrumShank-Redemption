...
def dax_based_risk_scoring(dataframe):
    """
    Implement a basic DAX-style risk scoring mechanism
    Criteria for high risk:
    1. More than 3 delays
    2. Cost overrun > 0
    3. SWAM compliance violations
    """
    # Calculate delays
    # Assuming these columns exist - adjust column names as needed
    ...
    if 'Ordered.Date' in df.columns and 'Requisition.Approved.Date' in df.columns:
        df['Ordered Date'] = pd.to_datetime(df['Ordered.Date'], errors='coerce')
        df['Fulfillment Delay'] = (df['Ordered Date'] - df['Req Approved']).dt.days
    else:
        df['Fulfillment Delay'] = 0

    # Calculate total delays
    df['Total Delays'] = df['Approval Delay'] + df['Fulfillment Delay']

    # SWAM Compliance check
    ...
    if available_swam:
        ...
        df['SWAM Compliance Violations'] = 0
    ...
    def calculate_risk_score(row):
        score = 0
        # More than 3 total delays
        if row['Total Delays'] > 3:
            score += 1

        # Cost overrun
        if row['Cost Overrun'] > 0:
            score += 1

        # SWAM Compliance violations
        if row['SWAM Compliance Violations'] > 0:
            score += 1

        return score

    # Apply risk scoring
    df['DAX Risk Score'] = df.apply(calculate_risk_score, axis=1)

    # Binary Risk Classification
    df['Is High Risk'] = df['DAX Risk Score'] >= 2

    # Detailed Risk Breakdown
    risk_summary = df.groupby('Is High Risk').agg({
        'DAX Risk Score': ['count', 'mean'],
        'Total Delays': 'mean',
        'Cost Overrun': 'mean',
        'SWAM Compliance Violations': 'mean'
    })

    print("\nDAX-Based Risk Scoring Summary:")
    print(risk_summary)

    # Detailed metrics
    total_suppliers = len(df)
    high_risk_suppliers = df['Is High Risk'].sum()
    high_risk_percentage = (high_risk_suppliers / total_suppliers) * 100
...
# Run the DAX-based risk scoring
result_df = dax_based_risk_scoring(df)

# SWAM Vendor Analysis
swam_columns = ['SWAM.Minority', 'SWAM.Woman', 'SWAM.Small', 'SWAM.Micro.Business']

# Filter for high-risk suppliers
high_risk_suppliers = result_df[result_df['Is High Risk']]

# SWAM Vendor Breakdown
def swam_vendor_breakdown(suppliers):
    print("SWAM Vendor Breakdown for High-Risk Suppliers:")
    for column in swam_columns:
        if column in suppliers.columns:
            swam_count = suppliers[suppliers[column] == 1].shape[0]
            swam_percentage = (swam_count / len(suppliers)) * 100
            print(f"{column}: {swam_count} suppliers ({swam_percentage:.2f}%)")

# Detailed SWAM Vendor Analysis
print("\nTotal High-Risk Suppliers:", len(high_risk_suppliers))
swam_vendor_breakdown(high_risk_suppliers)

# Comparative Analysis
print("\nComparative SWAM Vendor Analysis:")
for column in swam_columns:
    if column in result_df.columns:
        print(f"\n{column} Analysis:")
        print("Low Risk Suppliers:")
        low_risk_swam = result_df[(result_df['Is High Risk'] == False) & (result_df[column] == 1)].shape[0]
        low_risk_swam_percentage = (low_risk_swam / len(result_df[result_df['Is High Risk'] == False])) * 100
        print(f"  Count: {low_risk_swam} ({low_risk_swam_percentage:.2f}%)")

        print("High Risk Suppliers:")
        high_risk_swam = result_df[(result_df['Is High Risk'] == True) & (result_df[column] == 1)].shape[0]
        high_risk_swam_percentage = (high_risk_swam / len(result_df[result_df['Is High Risk'] == True])) * 100
        print(f"  Count: {high_risk_swam} ({high_risk_swam_percentage:.2f}%)")

# Optional: Visualization of risk distribution
#import matplotlib.pyplot as plt
#import seaborn as sns

#plt.figure(figsize=(10, 6))
#result_df['DAX Risk Score'].value_counts().plot(kind='bar')
#plt.title('Distribution of DAX Risk Scores')
...
# Identify Vendor/Supplier column
supplier_col = None
for potential_col in ['Vendor Name', 'Vendor', 'Supplier', 'Supplier Name']:
    if potential_col in df.columns:
        supplier_col = potential_col
        break

# Ensure we have a supplier column for later use
if supplier_col is None:
    df['Vendor Name'] = 'Unknown Vendor'
...
risk_probabilities = pipeline.predict_proba(X)[:, 1]

# Prepare export DataFrame for Tableau
tableau_export = df[[supplier_col] + features].copy()
tableau_export['Supplier Risk'] = df['Supplier Risk']
tableau_export['Risk Probability'] = risk_probabilities
tableau_export['Risk Score'] = df['Risk Score']

# Export to CSV for Tableau
tableau_export.to_csv('supplier_risk_tableau_export.csv', index=False)

# Save the model for potential future use
joblib.dump(pipeline, 'supplier_risk_model.joblib')

# Print some summary statistics
print("\nRisk Analysis Summary:")
print("Total Suppliers:", len(tableau_export))
print("High-Risk Suppliers:", tableau_export['Supplier Risk'].sum())
print("High-Risk Percentage: {:.2f}%".format(
    (tableau_export['Supplier Risk'].sum() / len(tableau_export)) * 100
))
...
sns.barplot(x='Importance', y='Feature', data=importance_df)
plt.title('Feature Importances in Supplier Risk')
plt.tight_layout()
plt.show()

print("\nExport complete. File 'supplier_risk_tableau_export.csv' is ready for Tableau import.")